...
class DialogWidget(QDialog):
    def __init__(self,parent=None):
        ...
        # self.defaultRadio = QRadioButton('default')
        # #self.defaultRadio.setChecked(True);
        # self.kkitRadio = QRadioButton('kkit')
    ...
    def validateAccept(self):
        ...
        if len(text) == 0:
            self.warning = QLabel("Model name cannot be empty!")
            self.layout.addWidget(self.warning, 1, 0, 1, -1)
        elif not re.match("^[a-zA-Z]+.*",text):
            self.warning = QLabel("Start special characters not allowed!")
            self.layout.addWidget(self.warning, 1, 0, 1, -1)
        else:
            self.accept()
        return False

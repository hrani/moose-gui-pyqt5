...
class GraphicalView(QGraphicsView):
    ...
    def __init__(self, modelRoot,parent,border,layoutPt,createdItem):
        ...
        self.stackOrder = self.sceneContainerPt.items(Qt.DescendingOrder)
    ...
    def editorMouseReleaseEvent(self,event):
        ...
        if clickedItemType == ITEM:
            ...
            if actionType == "move":
                            ...
                            if isinstance(parentPool,PoolBase):
                                if moose.exists(grpCmpt.mobj.path+'/'+parentPool.name):
                                    if moose.exists(grpCmpt.mobj.path+'/'+parentPool.name+'/'+movedGraphObj.name):
                                        ...
                                        reply = QMessageBox.question(self, "Moving the Object",'Do want to move the \'{movedGraphObj}\' to \'{grpCmpt}\''.format(movedGraphObj=movedGraphObj.mobj.name,grpCmpt=grpCmpt.mobj.name),
                ...
                QApplication.setOverrideCursor(QtGui.QCursor(Qt.ArrowCursor))
            if actionType == "delete":
                ...
                if self.state["move"]["happened"]:
                    ...
                    if isinstance(itemAtView,ComptItem) or isinstance(itemAtView, GRPItem):
                        ...
                        if itemAtView == None:
                            QMessageBox.information(None,'Dropping Not possible ','Dropping not allowed outside the compartment',QMessageBox.Ok)
                        else:
    ...
    def objectpullback(self,messgtype,item,movedGraphObj,xx,yy):
        ...
        QtGui.QApplication.setOverrideCursor(QtGui.QCursor(Qt.ArrowCursor))
    ...
    def drawExpectedConnection(self, event):
        ...
        if self.expectedConnection is None:
            ...
            self.expectedConnection.setPen(QPen(Qt.DashLine))
    ...
    def showConnector(self, item):
        ...
        for l in self.connectorlist.keys():
            ...
            if self.connectionSign != None:
                self.connectionSign.setFlag(QGraphicsItem.ItemIsSelectable,True)
    ...
    def keyPressEvent(self,event):
        key = event.key()
        self.removeConnector()
        if (key ==  Qt.Key_A and (event.modifiers() & Qt.ShiftModifier)): # 'A' fits the view to iconScale factor
        ...
        elif (key == Qt.Key_Less or key == Qt.Key_Minus):# and (event.modifiers() & Qt.ShiftModifier)): # '<' key. zooms-in to iconScale factor
            self.iconScale *= 0.8
            self.updateScale( self.iconScale )

        elif (key == Qt.Key_Greater or key == Qt.Key_Plus):# and (event.modifiers() & Qt.ShiftModifier)): # '>' key. zooms-out to iconScale factor
            self.iconScale *= 1.25
            self.updateScale( self.iconScale )

        elif (key == Qt.Key_Period): # '.' key, lower case for '>' zooms in
            self.scale(1.1,1.1)

        elif (key == Qt.Key_Comma): # ',' key, lower case for '<' zooms in
            self.scale(1/1.1,1/1.1)

        elif (key == Qt.Key_A):  # 'a' fits the view to initial value where iconscale=1
            self.iconscale = 1
            self.updateScale( 1 )
            self.fitInView(self.sceneContainerPt.itemsBoundingRect().x()-10,self.sceneContainerPt.itemsBoundingRect().y()-10,self.sceneContainerPt.itemsBoundingRect().width()+20,self.sceneContainerPt.itemsBoundingRect().height()+20,Qt.IgnoreAspectRatio)
    ...
    def deleteSelections(self,x0,y0,x1,y1):
        if( x1-x0 > 0  and y1-y0 >0):
            # self.rubberbandlist = self.sceneContainerPt.items(self.mapToScene(QtCore.QRect(x0, y0, x1 - x0, y1 - y0)).boundingRect(), Qt.IntersectsItemShape)
            ...
            self.rubberbandlist_qpolygon = self.sceneContainerPt.items(self.mapToScene(QtCore.QRect(x0, y0, x1 - x0, y1 - y0)).boundingRect(), Qt.IntersectsItemShape)
    ...
    def zoomSelections(self, x0, y0, x1, y1):
        ...
        self.fitInView(QtCore.QRectF(p0, p1), Qt.KeepAspectRatio)
    ...
    #     print self.viewBaseType, " event ",event
    #     '''
    #     if self.viewBaseType == "editorView":

...
def calculateChildBoundingRect(compt):
    ...
    for l in compt.childItems():
        ...
        if not isinstance(l,QtSvg.QGraphicsSvgItem):
            if (not isinstance(l,QGraphicsPolygonItem)):
        ...
        if (isinstance(l,PoolItem) or isinstance(l,EnzItem)):
            ...
            for ll in l.childItems():
                ...
                '''
                if ( (not isinstance(ll,QGraphicsPolygonItem)) and 
                     (not isinstance(ll,QGraphicsRectItem)) and 
                     (not isinstance(ll,QGraphicsEllipseItem))
                    ):
                    ypos.append(l.pos().y()+ll.pos().y()+ll.boundingRect().bottomRight().y())
    if xpos and ypos:
        calculateRectcompt = QRectF(min(xpos),min(ypos),(max(xpos)-min(xpos)),(max(ypos)-min(ypos)))
    else:

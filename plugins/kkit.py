...
class KkitPlugin(MoosePlugin):
    ...
    def __init__(self, *args):
        ...
        if not hasattr(self,'SaveModelAction'):
            ...
            self.saveModelAction.setShortcut(QApplication.translate("MainWindow", "Ctrl+S", None, 0))
    ...
    def SaveModelDialogSlot(self):
        ...
        filename,filter_ = QFileDialog.getSaveFileName(None,'Save File',dirpath,';;'.join(filters))
        extension = ""
        ...
        if filename:
            ...
            if filters[str(filter_)] == 'SBML':
                ...
                if writeerror == -2:
                    #QMessageBox.warning(None,'Could not save the Model','\n WriteSBML :  This copy of MOOSE has not been compiled with SBML writing support.')
                    QMessageBox.warning(None,'Could not save the Model',consistencyMessages)
                elif writeerror == -1:
                    QMessageBox.warning(None,'Could not save the Model','\n This model is not valid SBML Model, failed in the consistency check')
                elif writeerror == 1:
                    QMessageBox.information(None,'Saved the Model','\n File saved to \'{filename}\''.format(filename =filename+'.xml'),QMessageBox.Ok)
                elif writeerror == 0:
                     QMessageBox.information(None,'Could not save the Model','\nThe filename could not be opened for writing')
                ...
                if written == False:
                    ...
                    if error == "":
                        QMessageBox.information(None,'Saved the Model','\n File saved to \'{filename}\''.format(filename =filename+'.g'),QMessageBox.Ok)
                    else:
                        status = QtCore.QString("File saved but %2").arg(error);
                        QMessageBox.information(None,'Saved the Model but ...','{error}'.format(error=error),QMessageBox.Ok)
...
class KkitEditorView(MooseEditorView):
    ...
    def __init__(self, plugin):
        ...
        # if not hasattr(self,'SaveModelAction'):
        #     #self.fileinsertMenu.addSeparator()
        #     self.saveModelAction = QAction('SaveToGenesisFormat', self)
        #     self.saveModelAction.setShortcut(QApplication.translate("MainWindow", "Ctrl+S", None, QApplication.UnicodeUTF8))
    ...
    #             else:
    #                  QMessageBox.information(None,'Saved the Model','\n File Saved to \'{filename}\''.format(filename =filename),QMessageBox.Ok)
    '''
    def getToolPanes(self):
...
class  KineticsWidget(EditorWidgetBase):
    ...
    def groupChildrenBoundingRect(self):
        for k, v in self.qGraGrp.items():
            ...
            v.setPen(QtGui.QPen(QtGui.QColor(grpcolor), self.comptPen, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
    ...
    def comptChildrenBoundingRect(self):
        for k, v in self.qGraCompt.items():
            ...
            v.setRect(rectcompt.x()-10,rectcompt.y()-10,(rectcompt.width()+20),(rectcompt.height()+20))
            v.setPen(QtGui.QPen(QtGui.QColor(66,66,66,100), self.comptPen, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
    ...
    def drawLine(self,srcdes_list,arrow):
        ...
        pen = QtGui.QPen(QtCore.Qt.green, 0, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin)
